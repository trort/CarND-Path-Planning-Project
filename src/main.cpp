...
vector<double> map_xy_to_ref(double x, double y, double ref_x, double ref_y, double ref_yaw){
  double shift_x = x - ref_x;
  double shift_y = y - ref_y;
  double ptx = shift_x * cos(0 - ref_yaw) - shift_y * sin(0 - ref_yaw);
  double pty = shift_x * sin(0 - ref_yaw) + shift_y * cos(0 - ref_yaw);
  return {ptx, pty};
}

vector<double> car_xy_to_map(double x, double y, double ref_x, double ref_y, double ref_yaw){
...
vector<vector<double>> generate_path(json j, vector<vector<double>> map_waypoints){
  ...
  vector<double> map_waypoints_s = map_waypoints[2];
  vector<double> map_waypoints_dx = map_waypoints[3];
  vector<double> map_waypoints_dy = map_waypoints[4];
  ...
  int lane = 1;
  double ref_vel; //49.5 / 2.24; // convert mph to m/s
  if (prev_size < 2) {
    ref_vel = car_speed / 2.24;
  }
  else {
    double vx = ((double)previous_path_x[prev_size - 1] - (double)previous_path_x[prev_size - 2]) / 0.02;
    double vy = ((double)previous_path_y[prev_size - 1] - (double)previous_path_y[prev_size - 2]) / 0.02;
  ...
  for (int i = 0; i < sensor_fusion.size(); ++i) {
    ...
    if (ss_d < (2 + 4 * lane + 2) && ss_d > (2 + 4 * lane - 2)) {
      double ss_speed = sqrt(ss_vx * ss_vx + ss_vy * ss_vy);
      double check_car_s = ss_s + prev_size * 0.02 * ss_speed;
  ...
  // starting points as either the current state or the end of previous plan
  double ref_x = car_x;
  double ref_y = car_y;
  ...
  // debug
  cout << car_x << " " << car_y << " " << car_s << " " << car_d
       << " " << next_x_vals[0] << " " << next_y_vals[0] << " " << ref_vel << endl;
